...
function extract_avdb_data {
    ...
    cmd="grep -P \"^22\\t2007\" $ANNOVAR_HUMAN_DB_DIR/$annovar_file_name >> $script_dir/test_$annovar_file_name"
    eval_cmd "$cmd"
}
...
function generate_empty_vcf {
    ...
    cmd="tabix $input_vcf 22:20070171-20079973 | cut -f1-5 | awk -F'\t' '{ printf \"%s\t.\t.\t.\n\", \$0 }' >> $output_vcf"
    eval_cmd "$cmd"
}
...
function generate_gtz_vcf {
    ...
    cmd="tabix -h $input_vcf 22:20070171-20079973 > $output_vcf"
    eval_cmd "$cmd"
