...
function extract_avdb_data {
    ...
    cmd="grep -P \"^6\\t78171\" $ANNOVAR_HUMAN_DB_DIR/$annovar_file_name >> $script_dir/test_$annovar_file_name"
    eval_cmd "$cmd"
    cmd="grep -P \"^6\\t78172\" $ANNOVAR_HUMAN_DB_DIR/$annovar_file_name >> $script_dir/test_$annovar_file_name"
    eval_cmd "$cmd"
    cmd="grep -P \"^18\\t28610\" $ANNOVAR_HUMAN_DB_DIR/$annovar_file_name >> $script_dir/test_$annovar_file_name"
    eval_cmd "$cmd"
    cmd="grep -P \"^18\\t28611\" $ANNOVAR_HUMAN_DB_DIR/$annovar_file_name >> $script_dir/test_$annovar_file_name"
    eval_cmd "$cmd"
    cmd="grep -P \"^X\\t48690\" $ANNOVAR_HUMAN_DB_DIR/$annovar_file_name >> $script_dir/test_$annovar_file_name"
    eval_cmd "$cmd"
}
...
function generate_empty_vcf {
    ...
    cmd="tabix $input_vcf 6:78171940-78172992 | cut -f1-5 | awk -F'\t' '{ printf \"%s\t.\t.\t.\n\", \$0 }' >> $output_vcf"
    eval_cmd "$cmd"
    cmd="tabix $input_vcf 18:28610987-28611790 | cut -f1-5 | awk -F'\t' '{ printf \"%s\t.\t.\t.\n\", \$0 }' >> $output_vcf"
    eval_cmd "$cmd"
    cmd="tabix $input_vcf X:48690000-48691000 | cut -f1-5 | awk -F'\t' '{ printf \"%s\t.\t.\t.\n\", \$0 }' >> $output_vcf"
    eval_cmd "$cmd"
}
...
function generate_gtz_vcf {
    ...
    cmd="tabix -h $input_vcf 6:78171940-78172992 > $output_vcf"
    eval_cmd "$cmd"
    cmd="tabix $input_vcf 18:28610987-28611790 >> $output_vcf"
    eval_cmd "$cmd"
    cmd="tabix $input_vcf X:48690000-48691000 >> $output_vcf"
    eval_cmd "$cmd"
