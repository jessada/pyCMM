...
from pycmm.settings import MUTREPDB_CREATE_JOB_SETUP_FILE_DESCRIPTION
...
from pycmm.proc.mutrep.mutrep import DNARegion
from pycmm.proc.mutrep.mutrep import create_jobs_setup_file
from pycmm.app import display_configs

def app_pycmm_seq_report(*args, **kwargs):
    mylogger.getLogger(__name__)

    func_name = sys._getframe().f_code.co_name
    mc = MutRepController(jobs_setup_file=kwargs['jobs_setup_file'])
    ...
    display_configs(func_name,
                    MUTREPDB_SEQ_REPORT_DESCRIPTION,
                    kwargs,
                    mc,
                    custom_params=custom_params,
                    )
    mc.gen_report(report_regions=report_regions, out_file=out_file)
    mylogger.getLogger(__name__)
    disp.new_section_txt("F I N I S H <" + func_name + ">")

def app_pycmm_seq_reports(*args, **kwargs):
    mylogger.getLogger(__name__)
...
def app_pycmm_mutrepdb_create_jobs_setup_file(*args, **kwargs):
    mylogger.getLogger(__name__)
    func_name = sys._getframe().f_code.co_name
    ...
    disp.show_config(app_description=MUTREPDB_CREATE_JOB_SETUP_FILE_DESCRIPTION,
    ...
    if kwargs['db_file'] is not None:
        layout_params['annotated database file (-A)'] = kwargs['db_file']
