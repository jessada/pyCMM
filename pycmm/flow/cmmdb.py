...
JOBS_SETUP_PATIENTS_LIST_KEY = "PATIENTS_LIST"
...
class CMMDBPipeline(JobManager):
    ...
    @property
    def vcf_region(self):
        if JOBS_SETUP_VCF_REGION_KEY in self.__jobs_info:
            return str(self.__jobs_info[JOBS_SETUP_VCF_REGION_KEY])
        else:
            return None

    @property
    def patients_list(self):
        if JOBS_SETUP_PATIENTS_LIST_KEY in self.__jobs_info:
            return str(self.__jobs_info[JOBS_SETUP_PATIENTS_LIST_KEY])
        else:
            return None
...
def create_jobs_setup_file(dataset_name,
    ...
    if patients_list is not None:
        job_setup_document[JOBS_SETUP_PATIENTS_LIST_KEY] = patients_list
